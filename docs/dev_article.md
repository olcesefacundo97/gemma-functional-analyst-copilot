...
The default hosted model is configured as `gemma-3-27b-it`, the Gemma model name documented by Google for Gemma generation through the Gemini API. The backend abstraction can also run in demo mode for local portfolio reviews or public demos where no API key is configured.
...
API keys are generated in [Google AI Studio](https://aistudio.google.com/) and should never be committed. For a static public demo or local review mode, set `AI_PROVIDER=demo`. The public deployment may run in either mode depending on the configured Render environment.
...
The response should include `"model":"gemma-3-27b-it"` and a generated deliverable. Google AI Studio free tier quota, rate limits, invalid keys, unavailable models, and timeouts are handled as clean API errors by the FastAPI backend.
